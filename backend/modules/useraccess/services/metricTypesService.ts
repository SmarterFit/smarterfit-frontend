...
export const metricTypeService = {
   ...
   /**
    * Busca um tipo de métrica pelo nome
    */
   getByName(name: string): Promise<MetricTypeResponseDTO> {
      return apiRequest<MetricTypeResponseDTO>({
         method: "get",
         path: `/metrica/tipo/nome/${name}`,
      });
   },
   ...
   updateById(
      id: string,
      payload: MetricTypeRequestDTO
   ): Promise<MetricTypeResponseDTO> {
      return apiRequest<MetricTypeResponseDTO, MetricTypeRequestDTO>({
         method: "put",
         path: `/metrica/tipo/${id}`,
         data: payload,
